...
import { getMyAttendance } from '../features/student-portal/api'
import {
  formatDateTime,
    ...
    return (
      <div className="rounded-3xl border border-rose-200 bg-white p-6 shadow-sm">
        ...
        <h1 className="mt-4 text-2xl font-semibold tracking-tight text-slate-950">
          Your attendance records could not be fetched
        </h1>
      </div>
    )
  }
    ...
    <section className="space-y-6">
      <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        ...
        <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600">
          See how each class session was marked and review any recorded notes.
        </p>
      </div>
      ...
      {attendance.length === 0 ? (
        <div className="rounded-3xl border border-slate-200 bg-white p-10 text-center shadow-sm">
          ...
          <p className="mt-3 text-sm leading-6 text-slate-600">
            Attendance records will appear here after your classes are marked.
          </p>
        </div>
      ) : (
